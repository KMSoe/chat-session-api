...
use Modules\CRM\App\Http\Requests\ItemTypeRequest;
...
class ItemTypeController extends Controller
{
    ...
    {
        $item_types = $this->service->paginate($request->all());

        // if ($request->export) {
        //     $format = strtolower($request->format) ?? 'excel';
        //     switch ($format) {
        //         case 'excel':
        //             return Excel::download(new ContactExport($item_types), 'item_types.xlsx');
        //             break;
        //         case 'csv':
        //             return Excel::download(new ContactExport($item_types), 'item_types.csv');
        //             break;
        //         default:
        //             return Excel::download(new ContactExport($item_types), 'item_types.xlsx');
        //             break;
        ...
        return response()->json([
            'status' => true,
            'data'   => [
                'table_view_id' => TableView::fromName('item_type'),
    ...
    public function store(ItemTypeRequest $request)
    {
        $request->validated();
        try {
            $item_type = $this->service->create($request->all());

            return response()->json([
                'status'  => true,
                'data'    => [
                    'item_type' => new ItemTypeResource($item_type),
                ],
    ...
    public function update(ItemTypeRequest $request, $id)
    {
        $request->validated();
        try {
            $item_type = $this->service->update($id, $request->all());
