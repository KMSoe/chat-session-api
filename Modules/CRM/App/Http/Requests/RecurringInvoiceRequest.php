...
class RecurringInvoiceRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return[
            ...
            'recurring_level' => $isUpdate ? ['nullable', Rule::in(['item', 'invoice'])] : ['required', Rule::in(['item', 'invoice'])],
            'start_date' => $isUpdate ? 'nullable|date' : 'nullable|required_if:recurring_level,invoice|date',
            'end_date' => 'nullable|date|after_or_equal:start_date',
            'recurring_interval' => $isUpdate ? 'nullable|integer|min:0' : 'nullable|required_if:recurring_level,invoice|integer|min:0',
            'recurrence_frequency' => $isUpdate ? ['nullable', Rule::in(['day', 'week', 'month', 'year'])] : ['nullable', 'required_if:recurring_level,invoice', Rule::in(['day', 'week', 'month', 'year'])],
            ...
            'preference' => $isUpdate ? 'nullable|in:draft,create_and_send' : 'required|in:draft,create_and_send',
            ...
            'items.*.start_date' => $isUpdate ? 'nullable|date' : 'nullable|required_if:recurring_level,item|date',
            'items.*.end_date' => 'nullable|date|after_or_equal:items.*.start_date',
            'items.*.recurring_interval' => $isUpdate ? 'nullable|integer|min:0' : 'nullable|required_if:recurring_level,item|integer|min:0',
            'items.*.recurrence_frequency' => $isUpdate ? ['nullable', Rule::in(['day', 'week', 'month', 'year'])] : ['nullable', 'required_if:recurring_level,item', Rule::in(['day', 'week', 'month', 'year'])],
